...
pub mod agent_identity {
    ...
    pub fn register_agent(
        ...
        identity.name = name;
        identity.capabilities = 0; // Initial capabilities always 0
    ...
    // RESTRICTED: Only the DAO/Protocol Admin can verify and record task completion.
    pub fn record_task_completion(ctx: Context<RecordTask>) -> Result<()> {
         let identity = &mut ctx.accounts.agent_identity;
         let config = &ctx.accounts.config;
         
         // Verify signer is admin
         require!(ctx.accounts.authority.key() == config.admin, ErrorCode::Unauthorized);
         
         identity.task_count += 1;
         msg!("Task Recorded for Agent: {} (Total: {})", identity.name, identity.task_count);
         Ok(())
    }
...
pub struct RegisterAgent<'info> {
    ...
    /// Must sign to prove ownership/consent
...
pub struct Heartbeat<'info> {
    #[account(mut)]
    pub agent_wallet: Signer<'info>, // The Bot signs

    #[account(
        mut,
        seeds = [b"agent-id", agent_wallet.key().as_ref()],
        bump = agent_identity.bump,
        constraint = agent_identity.agent_wallet == agent_wallet.key()
    )]
    pub agent_identity: Account<'info, AgentIdentity>,
}
...
pub struct RecordTask<'info> {
    #[account(mut)]
    pub authority: Signer<'info>, // Must be DAO Admin
